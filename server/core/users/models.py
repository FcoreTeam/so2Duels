...
class UserManager(BaseUserManager):
    def create_user(self, username, password=None, email=None, **other_extra_fields):
        if self.model.objects.filter(username=username).exists():
            raise ValidationError("Username already exists")
        if not username:
            raise ValueError('The Username field must be set')
        
        user = self.model(username=username, **other_extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, username, password=None, email=None, **other_extra_fields):
        other_extra_fields.setdefault('is_staff', True)
        other_extra_fields.setdefault('is_superuser', True)
        other_extra_fields.setdefault('is_active', True)

        if other_extra_fields.get('is_staff') is not True:
            raise ValueError('Superuser must have is_staff=True.')
        if other_extra_fields.get('is_superuser') is not True:
            raise ValueError('Superuser must have is_superuser=True.')

        return self.create_user(username, password=password, **other_extra_fields)
...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    created_at = models.IntegerField(default=get_current_unix_time)
